 ...
 <style>
	...
	.btn-edit button:hover{
		...
		color:#fff;
			...
			<table border="1" style="width: 100%; border: 1px; color:#fff;">
				<thead>
					<tr>
						<td>STT</td>
						<td>MenuID</td>
				...
				<tbody>
					<c:forEach items="${menulist }" var="item" varStatus="status">
						<tr>
							...
							<td>${item.description }</td>
							<td>${item.categoryId }</td>
							<td><a href="javascript:void()" class="btn-edit"><button>Edit</button></a><input type="hidden" class="id-edit" name="name-edit" value="${item.menuId }"></td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
